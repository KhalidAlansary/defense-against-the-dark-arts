<template>
  <div class="mt-16">
    ...
    <Transition name="slide-fade" mode="out-in">
      <div v-if="!generated" key="button" class="flex flex-col items-center justify-center pt-16 min-h-[200px]">
        <button
        ...
        <div class="rounded-xl border border-white/10 bg-white/[0.03] overflow-hidden">
          ...
          <div class="border-b border-white/10 last:border-b-0">
            ...
            <div
              ...
              :class="highlightedSysReq ? 'bg-[#f9996c]/10 ring-2 ring-[#f9996c]/40' : 'hover:bg-white/[0.03]'"
              @click="sysReqOpen = !sysReqOpen"
            >
              <div class="flex items-center gap-2 min-w-0">
                <div class="flex h-4 w-4 shrink-0 items-center justify-center opacity-60">
                  <svg v-if="sysReqOpen" class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="m6 9 6 6 6-6" />
                  </svg>
                ...
                <span class="inline-flex items-center rounded-md border border-white/10 bg-white/[0.06] px-2 py-0.5 font-mono text-[11px] font-medium shrink-0">SYS-REQ-001</span>
                <span class="text-sm leading-snug truncate">Limit battery charge current when cell temperature exceeds 45°C.</span>
              </div>
              <div class="flex shrink-0 items-center gap-2 text-sm">
                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-medium border border-[#f9996c]/20 bg-[#f9996c]/10 text-[#f9996c]">ASIL D</span>
                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[11px] font-medium border border-white/10 bg-white/[0.06]" :class="acceptedCount === 1 ? 'text-[#95b9ba] border-[#95b9ba]/20 bg-[#95b9ba]/10' : 'opacity-60'">{{ acceptedCount }}/1 accepted SW reqs</span>
              </div>
            </div>
            ...
            <div v-if="sysReqOpen" class="border-t border-white/10 bg-white/[0.02]">
              <div class="p-3 pb-6">
                <div class="relative pl-12 pt-1">
                  <div class="absolute left-6 top-0 w-4 h-7 border-l border-b border-white/10 rounded-bl-xl" />
                  <div
                  ...
                  >
                    <div class="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-[#f9996c]/80 to-[#f9996c]/40 rounded-l-xl opacity-90" />

                    <div class="flex items-start justify-between gap-3">
                      <div class="flex flex-col gap-1.5 min-w-0">
                        <div class="flex items-center gap-2">
                          <span class="inline-flex items-center rounded-md border border-white/10 bg-white/[0.06] px-2 py-0.5 font-mono text-[11px] font-medium">SW-REQ-001</span>

                          <div class="relative inline-flex" @click.stop="refinesOpen = !refinesOpen">
                            <button
                            ...
                            >
                              <div class="flex flex-col gap-2">
                                <span class="text-[11px] font-medium opacity-50 uppercase tracking-wider">Refines</span>
                        ...
                        <p class="text-sm leading-snug max-w-full line-clamp-2">
                          When any cell temperature exceeds 45°C, the software shall reduce the battery charge current limit to a value less than or equal to the previously active limit within 10 ms.
                        </p>
                      </div>
                      ...
                      <span class="absolute bottom-3 right-3 text-[12px] text-[#f9996c] opacity-0 group-hover/card:opacity-100 transition-opacity">
                        View details
                      </span>
                    </div>
                  </div>
    ...
    <Teleport to="body">
      ...
      >
        ...
        <div class="relative w-full max-w-lg rounded-xl border border-white/10 bg-[#1a1a1a] shadow-2xl flex flex-col max-h-[85vh] overflow-hidden">
          <!-- Header -->
          <div class="px-6 py-4 border-b border-white/10 bg-white/[0.02]">
            <div class="flex items-center justify-between gap-3 mb-2">
              <span class="inline-flex items-center rounded-md border border-white/10 bg-white/[0.06] px-2 py-0.5 font-mono text-xs font-medium shrink-0">SW-REQ-001</span>
              <div class="flex items-center gap-2">
                ...
                <div class="relative inline-flex" @click.stop>
                  ...
                  <div
                    ...
                    @click.stop
                  >
                    <button class="flex w-full items-center gap-2 px-3 py-1.5 text-xs cursor-pointer hover:bg-white/[0.06] transition-colors" @click.stop="handleModalEdit">
                      <svg class="w-3.5 h-3.5 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /></svg>
                      Edit
                    </button>
                    <div class="border-t border-white/10 my-1" />
                    <button class="flex w-full items-center gap-2 px-3 py-1.5 text-xs cursor-pointer hover:bg-white/[0.06] transition-colors" @click.stop="handleModalReview">
                      <svg class="w-3.5 h-3.5 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.86 9.86 0 0 1-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8Z" /></svg>
                      Review
                    </button>
                  </div>
                </div>
                <button class="opacity-60 hover:opacity-100 cursor-pointer bg-transparent hover:bg-white/[0.06] rounded-md p-1.5 w-7 h-7 flex items-center justify-center" @click="showModal = false">
                  <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div v-if="modalMode === 'view'" class="overflow-y-auto p-6 space-y-3">
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div class="rounded-lg bg-white/[0.03] px-4 pt-3 pb-4">
                <p class="text-[10px] font-normal uppercase tracking-widest opacity-40 mb-3">Identity</p>
                <div class="flex min-w-0 items-baseline gap-3 py-2 border-b border-white/[0.06] last:border-0 last:pb-0 text-sm">
              ...
              <!-- Verification -->
              <div class="rounded-lg bg-white/[0.03] px-4 pt-3 pb-4">
                <p class="text-[10px] font-normal uppercase tracking-widest opacity-40 mb-3">Verification</p>
                <div class="flex min-w-0 items-baseline gap-3 py-2 border-b border-white/[0.06] last:border-0 last:pb-0 text-sm">
            ...
            <!-- Traceability -->
            <div class="rounded-lg bg-white/[0.03] px-4 pt-3 pb-4">
              <p class="text-[10px] font-normal uppercase tracking-widest opacity-40 mb-3">Traceability</p>
              <div class="flex min-w-0 items-start gap-3 py-2 border-b border-white/[0.06] last:border-0 last:pb-0 text-sm">
                <span class="opacity-50 w-20 shrink-0 pt-0.5 text-xs">Refines</span>
                <div class="flex min-w-0 flex-1 flex-wrap gap-1.5">
                  <span class="inline-flex items-center rounded-md border border-white/10 bg-white/[0.06] px-2 py-0.5 font-mono text-xs font-medium">SYS-REQ-001</span>
                </div>
              </div>
            </div>
            ...
            <!-- Review comments -->
            <div class="rounded-lg bg-white/[0.03] px-4 pt-3 pb-4">
              <p class="text-[10px] font-normal uppercase tracking-widest opacity-40 mb-3">Review comments</p>
          ...
          <div v-if="modalMode === 'edit'" class="overflow-y-auto p-6 space-y-4">
            <div class="flex flex-col gap-1.5">
              ...
              <textarea class="w-full rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-sm resize-none min-h-20 outline-none focus:border-[#f9996c]/40 transition-colors" :value="'When any cell temperature exceeds 45°C, the software shall reduce the battery charge current limit to a value less than or equal to the previously active limit within 10 ms.'" />
            </div>
            <div class="grid grid-cols-2 gap-4">
              <div class="flex flex-col gap-1.5">
                <label class="text-xs opacity-50">Category</label>
                <input class="w-full rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-sm outline-none focus:border-[#f9996c]/40 transition-colors" value="Software" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="text-xs opacity-50">Safety level</label>
                <input class="w-full rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-sm outline-none focus:border-[#f9996c]/40 transition-colors" value="ASIL D" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="text-xs opacity-50">Domain</label>
                <input class="w-full rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-sm outline-none focus:border-[#f9996c]/40 transition-colors" value="Powertrain, Safety" />
              </div>
              <div class="flex flex-col gap-1.5">
                <label class="text-xs opacity-50">Verification methods</label>
                <input class="w-full rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-sm outline-none focus:border-[#f9996c]/40 transition-colors" placeholder="Comma-separated" value="Test" />
              </div>
            </div>
            <div class="flex items-center justify-between gap-2 pt-3 border-t border-white/10">
              <button class="text-xs opacity-60 hover:opacity-100 cursor-pointer px-3 py-1.5 rounded-md border border-white/10 bg-transparent hover:bg-white/[0.04] transition-colors" @click="modalMode = 'view'">Cancel</button>
              <button class="text-xs border border-[#f9996c]/40 bg-[#f9996c] text-[#0c0c0c] px-3 py-1.5 rounded-md cursor-pointer hover:brightness-110 transition-all" @click="modalMode = 'view'">Save changes</button>
            </div>
          </div>
          ...
          <div v-if="modalMode === 'review'" class="overflow-y-auto p-6 space-y-4">
            <div class="flex flex-col gap-1.5">
              ...
              <textarea class="w-full rounded-md border border-white/10 bg-white/[0.04] px-3 py-2 text-sm resize-none min-h-30 outline-none focus:border-[#f9996c]/40 transition-colors" placeholder="Write a new review comment..." />
            </div>
            <div class="flex items-center justify-between gap-2 pt-3 border-t border-white/10">
              <button class="text-xs opacity-60 hover:opacity-100 cursor-pointer px-3 py-1.5 rounded-md border border-white/10 bg-transparent hover:bg-white/[0.04] transition-colors" @click="modalMode = 'view'">Cancel</button>
              <button class="text-xs border border-[#f9996c]/40 bg-[#f9996c] text-[#0c0c0c] px-3 py-1.5 rounded-md cursor-pointer hover:brightness-110 transition-all" @click="modalMode = 'view'">Submit review</button>
            </div>
          </div>
        </div>
      </div>
...
const statusLabel = computed(() => statusOptions.find(o => o.value === currentStatus.value)?.label ?? "Draft")

const statusDotClass = computed(() => statusOptions.find(o => o.value === currentStatus.value)?.dotClass ?? "bg-white/40")
...
function highlightSysReq() {
  ...
  glowTimer = setTimeout(() => { highlightedSysReq.value = false }, 2000)
}
